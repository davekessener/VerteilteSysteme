...
public class Start extends Application
{
	...
	public void start(Stage primary) throws Exception
	{
		try
		{
			...
			Option o_host = (new OptionBuilder("host")).setShortcut("h").hasValue(true).setForced(true).build();
			Option o_port = (new OptionBuilder("port")).setShortcut("p").hasValue(true).setDefault("1099").build();
			Option o_id = (new OptionBuilder("id")).setShortcut("n").hasValue(true).build();
			Option o_extended = (new OptionBuilder("extended")).setShortcut("x").build();
			Parser parser = new Parser(o_client, o_server, o_host, o_port, o_id, o_extended);
			...
			if(args.hasArgument(o_client) && !args.hasArgument(o_id))
				throw new IllegalArgumentException("Insufficient args given: Client ID missing!");
			...
			if(args.hasArgument(o_server))
			{
				...
				if(args.hasArgument(o_extended))
				{
					reg.rebind(SERVICE_CONFIG, (MessageServiceConfiguration) stub);
				}
			...
			{
				Properties.set(Properties.CLIENT_ID, args.getArgument(o_id));

				Registry reg = LocateRegistry.getRegistry(host, port);
				MessageService chat = (MessageService) reg.lookup(SERVICE_CHAT);

				if(args.hasArgument(o_extended))
				{
					MessageServiceConfiguration config = (MessageServiceConfiguration) reg.lookup(SERVICE_CONFIG);
					
					mApplication.addComponent(new ConfigurationComponent(config));
				}
				
				mApplication.addComponent(new RezeptionComponent(chat));
				mApplication.addComponent(new RedaktionComponent(chat));
...
//		System.setProperty("java.security.policy", "file:./security.policy");
	...
	private static final String SERVICE_CONFIG = "MessageServiceConfigure";
